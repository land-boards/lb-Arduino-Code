...
uint8_t testProto16(void)
{
  ...
  uint8_t testDigio32Card(void)
  {
    ...
    {
      BlackPillI2CMux.setI2CChannel(UUT_CARD_MUX_CH);		// Write out from UUT card
      Dio32.digitalWrite(port, LOW);
      BlackPillI2CMux.setI2CChannel(TEST_STN_INT_MUX_CH);	// Read in from Test Station
    ...
    BlackPillI2CMux.setI2CChannel(UUT_CARD_MUX_CH);			// Leave the UUT as inputs when done
    for (port = 0; port < 32; port++)
  ...
  uint8_t extLBTestOptoIn8(void)
  {
    ...
    BlackPillI2CMux.setI2CChannel(TEST_STN_INT_MUX_CH);	// Loop sets it back to Test Station before the end
    ...
    { // Turning on LEDs should result in a low on the OptoIn8 card
      ...
      BlackPillI2CMux.setI2CChannel(TEST_STN_INT_MUX_CH);	// Leave LED off when done
      Dio32.digitalWrite(port, LOW);
    }
